...
1. **Weather Agent** (Port 10001)
   ...
   - Implementation: `WeatherAgentExecutor` extending `SimpleAgentExecutor`

2. **Airbnb Agent** (Port 10002)
   - Searches and recommends accommodations
   - Skills: Accommodation search, booking recommendations
   - Implementation: `AirbnbAgentExecutor` extending `SimpleAgentExecutor`
...
3. **Host Agent** (Port 8080)
   ...
   - Implementation: Uses `A2AAgentRegistry` to manage remote agents

## Key Concepts Demonstrated

### 1. SimpleAgentExecutor

The Weather and Airbnb agents demonstrate extending `SimpleAgentExecutor` to implement the `AgentExecutorLifecycle` interface:
...
This approach:
- Extends `SimpleAgentExecutor` which implements the A2A Java SDK `AgentExecutor` interface
...
The Host Agent demonstrates connecting to remote agents using the new A2A Agent architecture:

```java
// Option 1: Using A2AAgentRegistry
A2AAgentRegistry registry = new A2AAgentRegistry();
AgentCard agentCard = registry.registerAgent("weather", "http://localhost:10001/a2a");
A2AAgent weatherAgent = registry.getAgent("weather");

// Option 2: Direct DefaultA2AAgentClient creation
...
- **Registry support**: `A2AAgentRegistry` for centralized agent management
...
|--------|---------------------|-----------|
| **Agent Implementation** | Implements `AgentExecutor` from a2a-python SDK | Extends `SimpleAgentExecutor` implementing `AgentExecutorLifecycle` |
...
| **Agent Registry** | Manual tracking | `A2AAgentRegistry` for centralized agent management |
...
This example uses the **lifecycle-based approach** with `SimpleAgentExecutor`:

| Feature | Lifecycle-Based (This Example) |
|---------|-------------------------------|
| **Agent Definition** | Manual `AgentCard` bean |
| **Execution Method** | Extend `SimpleAgentExecutor` and implement `onExecute()` |
