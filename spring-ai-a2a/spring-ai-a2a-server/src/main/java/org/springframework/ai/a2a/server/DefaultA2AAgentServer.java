...
package org.springframework.ai.a2a.server;
...
import org.springframework.ai.a2a.server.agentexecution.AgentExecutorLifecycle;
 ...
 *
 * <p>
 * The server delegates task execution to {@link AgentExecutor} and manages the
...
public class DefaultA2AAgentServer implements A2AAgentServer {
	...
	public DefaultA2AAgentServer(AgentCard agentCard, AgentExecutor agentExecutor) {
	...
	private Map<String, Object> handleSendMessage(Map<String, Object> params, Object id) {
		try {
			...
			// All Spring AI agents implement AgentExecutorLifecycle which provides
			// synchronous execution
			A2AResponse response;
			if (this.agentExecutor instanceof AgentExecutorLifecycle lifecycle) {
				response = lifecycle.executeSynchronous(request);
			}
			else {
				throw new UnsupportedOperationException("AgentExecutor must implement AgentExecutorLifecycle. "
						+ "Found: " + this.agentExecutor.getClass().getName());
			}
